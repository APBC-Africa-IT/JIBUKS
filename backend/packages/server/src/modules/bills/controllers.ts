/**
 * Bills controllers -- HTTP layer only.
 */

import type { Request, Response } from "express";
import { createWriteBillSchema } from "@jibuks/domain";
...
export async function create(req: Request, res: Response): Promise<void> {
  const body = createWriteBillSchema.parse(req.body);
  const journal = await service.createWriteBill(
    {
