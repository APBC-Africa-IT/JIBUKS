/**
 * Cash sales controllers -- HTTP layer only.
 */

import type { Request, Response } from "express";
import { createCashSaleSchema } from "@jibuks/domain";
...
export async function create(req: Request, res: Response): Promise<void> {
  const body = createCashSaleSchema.parse(req.body);
  const journal = await service.createCashSale(
    {
